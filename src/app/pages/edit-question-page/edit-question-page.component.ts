...
export class EditQuestionPageComponent implements OnInit {
  ...
  updateQuestion(questionp: FormGroup) {
    ...
    let bn = 'j';
    this.questionUp.reponse.forEach((reponse, index) => {
      if (question.bonneReponse === 'reponse' + (index + 1)) {
        this.questionUp.reponse[index].estCorrect = true;
      } else {
        this.questionUp.reponse[index].estCorrect = false;
      }
    });

    this.questionService
      .updateQuestion(this.id, this.questionUp)
      .subscribe(() => {
